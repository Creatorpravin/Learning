...
class ActionHandler:
    ...
    system_configuration_manager:
        An instance of SystemConfigurationManager must be constructed and passed to the ActionHandler. This instance will 
                ...
                if self.__validate_message(message) == True:
                    try:
                        if self.__parsed_message[MessageDefinitions.DEVICE_ID_KEY] == self.__system_configuration_manager.get_device_id():
                            if self.__parsed_message[MessageDefinitions.SERVICE_CODE_KEY] in self.__service_manager_dictionary:
                                self.__service_manager_dictionary[self.__parsed_message[MessageDefinitions.SERVICE_CODE_KEY]].service_handler_function(self.__parsed_message)
                            else:
