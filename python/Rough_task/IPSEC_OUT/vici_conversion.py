...
class res:
    ...
    def _vici(self):
        ...
        for id, stat in enumerate(self.stats):                        
            self.statistics_dict[id] = json.dumps(stat, cls=ChiefNETJsonBinaryEncoder)
        return self.statistics_dict


statistics = (vici.Session()).list_sas()

result = res(stats=statistics)
result_dict = dict()
result_dict["connection"] = result._vici()
